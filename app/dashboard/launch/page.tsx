...
export default function LaunchPage() {
  ...
  async function handleExecute() {
    ...
    try {
      // Create project
      const { data: project, error: pErr } = await supabaseBrowser
        .from("projects")
        .insert({
          user_id: user.id,
      ...
      const payload: ExecutionRequestV1 = {
        protocolVersion: "1.0",
        requestId,
        tenantId: user.id,
        project: {
        ...
        capabilities: selected.map((s) => ({
          id: s,
          category: capabilityToCategory(s),
        })),
        inputs: {
  ...
  function capabilityToCategory(
    cap: string
  ): ExecutionRequestV1["capabilities"][number]["category"] {
    if (
      cap.includes("Strategy") ||
      cap === "BRD" ||
      cap === "GTM Strategy"
    )
      return "STRATEGY";
    ...
    <div className="min-h-screen p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <p className="text-sm text-neutral-600">
          Create artifacts using Foundry's orchestration.
        </p>
        ...
        <HandshakeModal
          ...
          run={async () => {
            try {
              ...
              const r = await fetch("/api/foundry/execute", {
                method: "POST",
                headers: {
